...
} elseif ($result1["http_code"] != 200 || $result[1]["body"] == null) { //Something went wrong, give cached data if possible, error entry otherwise.
	if (isSet($cachedJSON)) {
		$data["Events"] = json_decode($cachedJSON);
	} else {
		$data["Events"] = array(
...
foreach($worldCmpEventKeys as $cmpKey) {
	...
	if ($result1Cmp["http_code"] == 304) { //Nothing changed! Use the cached data!
		$data["Events"][] = json_decode($cachedJSON); 
	} elseif ($result1Cmp["http_code"] != 200 || $result[1]["body"] == null) { //Something went wrong, give cached data if possible, error entry otherwise.
		if (isSet($cachedJSON)) {
		$data["Events"][] = json_decode($cachedJSON);
		} else {
...
foreach ($data["Events"] as $event) {
	...
	} elseif ($result2["http_code"] != 200 || $result[1]["body"] == null) { //Something went wrong, give cached data if possible, error entry otherwise.
		if (isSet($cachedJSON)) {
		$tmpData["TeamList"] = json_decode($cachedJSON); 
		} else {
